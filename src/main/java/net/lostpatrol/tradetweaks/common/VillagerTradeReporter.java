package net.lostpatrol.tradetweaks.common;
...
import static net.lostpatrol.tradetweaks.util.Util.toRoman;
...
public class VillagerTradeReporter {
    ...
    private static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        for (AbstractVillager villager : villagers) {
            if (villager instanceof Villager v) {
                ...
                playerData.lastKnownOffers.put(villagerId, copyOffers(currentOffers));
            }
        }
    }
    ...
    public static void reportTrades(Villager villager, Player player, boolean usingWand) {
        ...
        Component professionName = Util.getProfessionComponent(data.getProfession());
        Component levelName = Util.getLevelComponent(data.getLevel());
        ...
        for (MerchantOffer offer : offers) {
            ItemStack buying1 = offer.getBaseCostA();
        ...
        if (!usingWand && hasMending) {
            Util.highlightVillagerWithEntity(villager, 600);
        }
    }
