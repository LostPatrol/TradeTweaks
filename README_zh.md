...
+ 自选交易界面复用原版村民交易界面的视觉与操作方式，左栏显示村民现有交易，右栏显示可替换交易；两栏均保留完整的原版边框与阴影，两侧滚动条均与完整列表进度一致，界面不会暂停单人游戏世界
+ 右栏顶部提供实时生效的 JEI 风格搜索框，会检索每项交易的全部买入和产出物品。普通文本匹配名称与提示，`@` 搜索模组名，`#` 搜索标签，`$` 搜索物品提示，`%` 搜索创造模式物品栏，`^` 搜索颜色，`&` 搜索资源 ID；空格表示同时满足，`|` 表示任选其一，`-` 排除关键词，引号可保留短语。按 `Tab` 聚焦搜索框，按回车或 `Esc` 退出输入
...
物品栏类进度会检查手杖实际保存的升级数据，因此无论制作还是通过其他方式获得升级后的绿宝石手杖，均会解锁“市场支配”。
